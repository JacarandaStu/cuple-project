...
| 学号      	| 姓名 	| 作业博客链接		| 具体分工		|
| --------- |:-----:|:-----------------:|:-------------:|
| 032004109 | 李焯春	| [点击这里](https://bbs.csdn.net/topics/608611659?spm=1001.2014.3001.6377) |对局流程设计、原型设计、素材收集、AI启发算法 |
...
**前端部分的若干重要代码片段**
1. 前端复现的选择函数（一般方法）。其主要思路是，遍历计算三条线的落子收益，使得落子后(我方分值 - 对方分值)达到最大，
对于同样收益的情况，默认选择剩余空位最多且靠前的位置。看起来很一般，但是实际游玩过程貌似还是运气成分更重要，和玩家PK的胜率相持。
> ![choose1](/images/code/choose1.png)
> ![choose2](/images/code/choose2.png)
> ![choose3](/images/code/choose3.png)
2. 托管功能的具体实现。大概分为几步，模拟投掷骰子、调用选择函数决定落子位置、落子并进行吃子操作、交换回合。
> ![AI1](/images/code/AI1.png)
> ![AI2](/images/code/AI2.png)
> ![AI3](/images/code/AI3.png)
3. 吃子操作处理逻辑。分玩家和机器操作两种情况，最终都是根据当前格子的id去和存储双方棋盘内点数值的全局变量的对应行切片比较有无相同数值，有则消除。
> ![eat1](/images/code/eat1.png)
> ![eat2](/images/code/eat2.png)
4. 计算分值。将棋盘分为3个片段，对应各个片段计算sum(num*num_times^2)，然后再计算三线分值总和。
...
|:---------:|:--------:|:-----------------------:|:----------------:|:-------------:|
| Planning	| 计划	| none	| 60	| 35	|
| Estimate	| 估计这个任务需要多少时间	| none	 | 60	| 15	|	
| Development	| 开发	|	web三要素、python提前模拟对局 |	3000 | 2400	|
| Analysis	| 需求分析 (包括学习新技术)	| none	| 40	| 45	|
| Design Spec	| 生成设计文档	| markdown	|	60 | 40	|
| Design Review	| 设计复审	| none	|	60 | 50	|
| Coding Standard	| 代码规范 (为目前的开发制定合适的规范)		|	IDEA自带提示 | 20	| 10	|
| Design	| 具体设计	| 即时设计	|	180 | 800	|
| Coding	| 具体编码	| html+css+js	|	1000 | 1600	|
| Code Review	| 代码复审	| none	| 120	|	10 |
| Test	| 测试（自我测试，修改代码，提交修改）	|	none |	180 | 0	|
| Reporting	| 报告	| video	|	120 | 50	|
| Test Report	| 测试报告	| none	|	10 | 5	|
| Size Measurement	| 计算工作量		| 计算器 | 30	| 10	|
| Postmortem & Process Improvement Plan	| 事后总结, 并提出过程改进计划		|	none | 40	| 10	|
| Sumarize	| 合计	| none	|	4980 |	5260 |
## 4.2 学习进度条（每周追加）（2分）
